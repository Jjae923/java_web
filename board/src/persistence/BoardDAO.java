...
public class BoardDAO {
	...
	// 게시글 삭제
	// drop from board where bno=? and password=?
	...
	public int replyArticle(BoardVO vo) {
		...
		try {
			...
			// 댓글 작업을 위한 원본글의 re_ref, re_seq, re_lev 가져오기
			...
			pstmt.setString(4, vo.getContent());
			pstmt.setString(5, vo.getAttach());
			pstmt.setInt(6, re_ref); // 원본 글의 re_ref와 동일
			pstmt.setInt(7, re_lev+1); // 원본 글의 re_lev+1
			pstmt.setInt(8, re_seq+1); // 원본 글의 re_seq+1
